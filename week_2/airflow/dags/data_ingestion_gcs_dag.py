...
# The environment variables for GCP set in the docker-compose file
PROJECT_ID = os.environ.get("GCP_PROJECT_ID")
BUCKET = os.environ.get("GCP_GCS_BUCKET")

dataset_file = "yellow_tripdata_2021-01.parquet"
# dataset_file = "yellow_tripdata_2021-01.csv" ## The file is no longer available there as a CSV (TODO: see if there's a way to get the .csv.gz file and use that instead)
...
with DAG(
    ...
    # The first operator is a BashOperator that downloads the dataset from the source
    download_dataset_task = BashOperator(
        task_id="download_dataset_task",
        bash_command=f"curl -sSL {dataset_url} > {path_to_local_home}/{dataset_file}",
    )

    # This operator is a PythonOperator that converts the CSV file to parquet format
