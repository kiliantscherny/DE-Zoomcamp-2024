...
local_workflow = DAG(
    "FHVHVDataIngestion",
    schedule_interval="0 6 1 1,6 *",  # Run at 6:00 AM on the 1st day of February and July
    start_date=datetime(2021, 1, 1),    # Start date of the DAG
    end_date=datetime(2021, 7, 1),      # End date of the DAG
...
URL_TEMPLATE = (
    URL_PREFIX + "fhvhv_tripdata_{{ execution_date.strftime('%Y-%m') }}.csv.gz"
)
OUTPUT_FILE_NAME = "fhvhv_{{ execution_date.strftime('%Y-%m') }}"
...
TABLE_NAME_TEMPLATE = "fhvhv_{{ execution_date.strftime('%Y_%m') }}"
